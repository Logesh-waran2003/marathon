...
export default function CertifiedBy() {
  ...
  return (
    <section ref={ref} className="relative border-y border-gray-100 bg-gradient-to-b from-white to-gray-50/50 py-12 sm:py-16 px-5 sm:px-8">
      <div className="mx-auto max-w-5xl">
        <p className={`text-center text-[10px] font-bold uppercase tracking-[0.3em] text-muted mb-8 sm:mb-10 ${visible ? "anim-fade-up" : "opacity-0"}`}>
        ...
        <div className="flex flex-wrap justify-center gap-4 sm:gap-6 lg:gap-8">
          {certs.map((c, i) => (
            <div
              key={c.name}
              className={`group flex items-center gap-4 rounded-2xl bg-white px-6 py-4 sm:px-8 sm:py-5 border border-gray-100 shadow-sm shadow-black/[0.03] transition-all duration-300 hover:shadow-md hover:shadow-primary/[0.06] hover:border-primary/15 hover:-translate-y-0.5 ${visible ? "anim-fade-up" : "opacity-0"}`}
              style={{ animationDelay: `${0.15 + i * 0.1}s` }}
            >
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-gradient-to-br from-primary/10 to-gold/10 flex items-center justify-center border border-primary/10 group-hover:border-primary/25 group-hover:scale-105 transition-all duration-300">
                {c.icon}
              </div>
              <div>
                <p className="text-sm sm:text-base font-bold text-accent">{c.name}</p>
                <p className="text-[10px] sm:text-xs text-muted leading-tight">{c.full}</p>
              </div>
            </div>
          ))}
        </div>
